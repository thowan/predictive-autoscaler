...
def main():
    ...
    while i <= len(series):
        ...
        model_fit = model.fit()
        x = model_fit.predict(start=n,end=n+params["window_future"])


        past_values = series[i-params["window_past"]:i]
        window_past = len(past_values)
        x = x.tolist() + past_values
        ...
        #Y.append(series[i-1])
        # p = np.percentile(series[i-5:i],95)
        # Y.append(p)
        print (downscale)
        print (scaleup)
        # Only scale when prediction has stablized somewhat
        
        if CPU_request - p > params["scale_down_buffer"] and scaleup > 20 and downscale > 10:
            if np.max(Y[-5:])-np.min(Y[-5:]) < 50:
                print("CPU request wasted")
                CPU_request = p + params["rescale_buffer"]
                rescale_counter += 1
                downscale = 0
        elif p - CPU_request > params["scale_up_buffer"]: 
            if np.max(Y[-5:])-np.min(Y[-5:]) < 50:
    ...
    #Y = model_fit.predict(start=len(series), end=len(series)+20)
    print(np.square(np.subtract(series,Y.pop())).mean() )
    X = range(len(Y))
    X = [x+test_last for x in X]
    
    ax1.plot(X, Y, 'o-', linewidth=1,label='HW prediction')
    slen, alpha, beta, gamma, n_preds = 12, 0.716, 0.029, 0.993, 48
    # series2 = triple_exponential_smoothing(series_small, slen, alpha, beta, gamma, n_preds)
    
    # series_X2 = range(len(series2))
    # ax1.plot(series_X2, series2, 'r.-', linewidth=1,label='Blog')


    xrequest = range(len(yrequest))
    xrequest = [x+test_last for x in xrequest]
    ax1.plot(xrequest,yrequest, '.-', linewidth=1,label='CPU requested')
