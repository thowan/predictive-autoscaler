...
def patch(client, requests, limits):
    ...
    dep = {"spec":{"template":{"spec":{"containers":[{"name":"nginx","resources":{"requests":{"cpu":int(requests)},"limits":{"cpu":int(limits)}}}]}}}}
    resp = v1.patch_namespaced_deployment(name='nginx-deployment',  namespace='default', body=dep)
    print("PATCHED request, limits:", int(requests), int(limits))
...
def animate(i):
    ...
    if cpu_metrics_server is not None and cpu_requests is not None:
        ...
        if current_step >= start_time: 
            ...
            model_fit = model.fit()
            if current_step > season_length*3:
                current_step = season_length*3

            x = model_fit.predict(start=current_step-1,end=current_step+2)
            ...
            # If predicted CPU will be low for a while, requested cpu is too high
            # if cpu_requests - p > 200:
            #     print("CPU request wasted")
            #     patch(client, p+25, 500)
            # elif p - cpu_requests > 100: 
            #     print("CPU request too low")
            #     patch(client, target+25, upperBound)
