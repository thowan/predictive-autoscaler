...
def main():
    ...
    scale_u_s = [3,5,10]
    scale_d_s = [3,5,10]
    #scale only after x steps after last scale
    scale_u_c = [10]
    scale_d_c = [10]
                ...
                for d in scale_d_s:
                    for e in scale_u_c:
                        for f in scale_d_c:
                            ...
                            params["scaleup_count"] = e
                            params["scaledown_count"] = f
                            rescale_counter = 0
                            scaleup = 0
                            downscale = 0
                            CPU_request = 500
                            ...
                            while i < len(series):
                                if i > 10:
                                    ...
                                    if CPU_request - p > params["scale_down_buffer"] and scaleup > params["scaleup_count"]  and downscale > params["scaledown_count"]:
                                        if np.max(series[i-params["scale_down_stable"]:i])-np.min(series[i-params["scale_down_stable"]:i]) < 50:
                                        ...
                                        if np.max(series[i-params["scale_up_stable"]:i])-np.min(series[i-params["scale_up_stable"]:i]) < 50:
                            ...
                            if rescale_counter < 12  and MSE < error: 
    ...
    noise = np.random.normal(0,10,len(series))
    series = [sum(x) for x in zip(noise, series)]
    series = [int(i) for i in series]
    add = np.arange(len(series))
    add = [x*0.6 for x in add]
    ...
    add = np.random.normal(1000,25,2)
    series[250:252]=add
